...
In this setup, the HDFS cluster is deployed on 3 nodes:
...
3. Install and configure HDFS on Node 0
	...
	```
4. Copy the Hadoop binaries from Node 0 to Node 1 and Node 2
	```sh
...
7. Start the HDFS cluster and test the setup
	...
	```
	Note: currently the version ID and cluster ID are hardcoded in the source code, so the LiteSys datanode shoould be recompiled each time the namenode is re-formated, which changes the version ID and cluster ID.