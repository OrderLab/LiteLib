...
MemcachedServer *main_server_ptr = nullptr;
...
MemcachedServer::MemcachedServer(const size_t &nthreads,
  ...
  for (int i = 0; i < nthreads; i++) {
    workers_.emplace_back(
        new Worker(lite_server_, backend_addr_, backend_port_));
    (**workers_.rbegin()).Run();
  }
  next_worker_ = workers_.begin();

  if (main_server_ptr) {
    throw std::runtime_error("multiple servers not supported");
  }
  main_server_ptr = this;
}
...
bool MemcachedServer::Run(const char *port) {
  int sfd;
...
void MemcachedServer::EventHandler(const evutil_socket_t fd, const short which,
                                   void *arg_conn) {
  ...
  main_server_ptr->DispatchNewConnection(new_conn_fd);
}

void MemcachedServer::DispatchNewConnection(const int sfd) {
  uint64_t buf = sfd;
