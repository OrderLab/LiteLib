...
if exp_type == 'lite':
  boot_command = ["taskset", "-c", "0,1,2,3", "ip", "net", "e", "testx", '/workspace/Memcached_codes/memcached_reference_model', '-t', '4', '-s', f"{LITE_SIZE}", '-p', "60001"]
  subprocess.Popen(boot_command, start_new_session=True)
  print(boot_command)
  ...
  boot_command = ["taskset", "-c", "0,1,2,3", "ip", "net", "e", "testx", "memcached.replica", "-p", "60001", "-d", "-u", "root", "--enable-shutdown", "-m", str(REPLICA_SIZE), "-t", "4"]
  subprocess.Popen(boot_command, start_new_session=True)
  print(boot_command)
elif exp_type != 'full':
