...
if(args_len != 9):        
        print("enter valid parameters:  load, trigger, duration of test, zipf_param, num_threads, trigger_offset, closed_loop_test, time_out, exp_type")
        exit()

def run_trace_replay(_trace_file_name, _num_threads, _result_file_name, _path_to_rsa_key, _memcached_host, _memcached_monitor_command):
    os.system("make") # to make sure latest changes are being used 
    run_TraceReplay_command = "./TraceReplay -t {}  {} -n {} -r {} -i {} -s {} -m \"{}\"".format(_trace_file_name, closed_loop_flag, _num_threads, _result_file_name, _path_to_rsa_key, _memcached_host, _memcached_monitor_command)
...
try:
    cmd = f"ssh -t -o StrictHostKeyChecking=no -i {path_to_rsa_key} -p 22 {user}@{memcached_host} \"python3 init.py {exp_type}\"" 
...
try:
    cmd = f"ssh -t -o StrictHostKeyChecking=no -i {path_to_rsa_key} -p 22 {user}@{memcached_host} \"{memcached_VM_command}\"" 
    out = subprocess.check_output(cmd , shell= True)
    print(out)
except subprocess.CalledProcessError as e:
...
trace_file_name = "traces/trace_file_" + str(float(_lambda)) + "_" + str(float(zipf_parameter)) + "_" + str(float(duration_of_test)) + ".txt"
results_directory = "results_warm_cache/" 
result_file_name = "result_" + str(float(_lambda)) + "_" + str(float(zipf_parameter)) + "_DUR_" + str(duration_of_test) + "_TRIGGER_" + str(trigger_time) + "_TMOUT_"+ str(timeout) +".txt"
result_file_path = results_directory + result_file_name
monitor_file_name = "monitor_" + str(float(_lambda)) + "_" + str(float(zipf_parameter)) + "_DUR_" + str(duration_of_test) + "_TRIGGER_" + str(trigger_time) + "_TMOUT_"+ str(timeout) +".txt"
memcached_monitor_command = "python3 monitor.py {} {}".format(duration_of_test, monitor_file_name)
trace_replay_thread = Thread(target= run_trace_replay, args=(trace_file_name, num_threads, result_file_path,path_to_rsa_key,f"{user}@{memcached_host}",memcached_monitor_command))
trace_replay_thread.start()  
# p = subprocess.Popen(["ssh", "-t", "-o", "StrictHostKeyChecking=no", "-i", path_to_rsa_key, "-p", "22", f"{user}@{memcached_host}", memcached_monitor_command],
...
# step 5: run trigger  
# print("skip trigger")
trigger_command = "python3 crash.py " + str(trigger_time) + " " + exp_type
print(trigger_command)
try:
    cmd = f"ssh -t -o StrictHostKeyChecking=no -i {path_to_rsa_key} -p 22 {user}@{memcached_host} \"{trigger_command}\"" 
    out = subprocess.check_output(cmd , shell= True)
    print(out)
except subprocess.CalledProcessError as e:
...
try:
    ...
    scp_command = "sshpass -p metastability scp -o StrictHostKeyChecking=no " + "{}@{}:{} ".format(connUser, connHost, connPath) + local_monitor_file
    #scp = subprocess.Popen(['scp', '-i', connPrivateKey, result_file_path, '{}@{}:{}'.format(connUser, connHost, connPath)])
    os.system(scp_command)
except CalledProcessError:
    print('ERROR: Connection to host failed!')

process_stats_command = "python3 collect_stats_over_time.py {} {} {} {} {} {}".format(result_file_path, _lambda, zipf_parameter, trigger_time,duration_of_test, local_monitor_file)
...
try:
    ...
    connUser = 'root'
    connHost = master_vm
    connPath = '/workspace/LoadGenerator/results_warm_cache/' + result_file_name  
    #os_command = f"ssh-keyscan {master_vm}  >> $HOME/.ssh/known_hosts"
    #os.system(os_command)
    # Use scp to send file from local to host.
    scp_command = "sshpass -p metastability scp -o StrictHostKeyChecking=no " +  result_file_path + " {}@{}:{}".format(connUser, connHost, connPath)
    #scp = subprocess.Popen(['scp', '-i', connPrivateKey, result_file_path, '{}@{}:{}'.format(connUser, connHost, connPath)])
    os.system(scp_command)
except CalledProcessError:
    print('ERROR: Connection to host failed!')
...
stats_file_name = "STATS_ARV_RATE_{}_ALPHA_{}_TRIGGER_{}_DUR_{}.txt".format(_lambda, zipf_parameter, trigger_time, duration_of_test)
