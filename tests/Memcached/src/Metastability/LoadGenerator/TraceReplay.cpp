...
{
    cerr << "Usage: " << argv0 << " -t traceFile [-r resultsFilename] [-u baseUrl] [-n numThreads] [-o] [-c] [-i pathToRSAKey] [-s SSHDest] [-m SSHCmd]" << endl;
    exit(-1);
}
...
int main(int argc, char** argv)
{
    ...
    */
    int opt = 0;
    string filename, path_to_rsa_key, ssh_dest, ssh_cmd;
    do {
        opt = getopt(argc, argv, "t:r:u:n:i:s:m:oc");
        switch (opt) {         
            case 't':
            ...
            case 'i':
                path_to_rsa_key.assign(optarg);
                break;

            case 's':
                ssh_dest.assign(optarg);
                break;

            case 'm':
                ssh_cmd.assign(optarg);
                break;
    ...
    if (fork() == 0) {
      ...
      start_time << " " << std::chrono::duration_cast<std::chrono::microseconds>((now + std::chrono::seconds(5)).time_since_epoch()).count();
      ssh_cmd += start_time.str();
      string _t("-t");
      string _o("-o");
      string _StrictHostKeyChecking("StrictHostKeyChecking=no");
      string _i("-i");
      string _p("-p");
      string _22("22");
      cerr << "ssh cmd: " << ssh_cmd << endl;
      char* argument_list[] = {&_t[0],
                               &_o[0],
                               &_StrictHostKeyChecking[0],
                               &_i[0],
                               &path_to_rsa_key[0],
                               &_p[0],
                               &_22[0],
      ...
      close(fd);   /* close fd */
      int status_code = execvp("ssh", argument_list);

      if (status_code == -1) {
        cerr << "ssh cmd failed" << endl;
        return 1;
      }
      return 0;
    }

    cout << "ssh cmd started" << endl;
