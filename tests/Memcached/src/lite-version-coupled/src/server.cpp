...
memcached::server::Server *main_server_ptr;

Server::Server(const size_t &nthreads, const size_t &max_item_count)
    : request_handler_(max_item_count) {
  ...
  for (int i = 0; i < nthreads; i++) {
    workers_.emplace_back(new Worker(request_handler_));
    (**workers_.rbegin()).Run();
  }
...
bool Server::Run(const char *port) {
  int sfd;
...
void Server::EventHandler(const evutil_socket_t fd, const short which,
                          void *arg_conn) {
...
void Server::DispatchNewConnection(const int sfd) {
  uint64_t buf = sfd;
