...
if [ "$id" == "3" ]; then
  if [ "$LOG_PREFIX" != "none" ]; then
    GLOG_stderrthreshold=0 GLOG_logtostderr=1 /workspace/tests/Memcached/src/lite-version-ascii/build/LiteMemcached -t 8 -s 20480 > $Dir/logs/$LOG_PREFIX.lite_memcached.log 2>&1 &
    ...
    GLOG_stderrthreshold=0 GLOG_logtostderr=1 /workspace/tests/Memcached/src/lite-version-ascii/build/LiteMemcached -t 8 -s 20480 &
  ...
  if [ "$LOG_PREFIX" != "none" ]; then
    ...
    GLOG_stderrthreshold=0 GLOG_logtostderr=1 /workspace/tests/Memcached/src/lite-version-ascii-embedded/build/LiteMemcached -t 8 -s 20480 > $Dir/logs/$LOG_PREFIX.lite_memcached.$id.log 2>&1 &
  else
