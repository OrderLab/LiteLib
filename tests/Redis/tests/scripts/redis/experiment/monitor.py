...
def monitor_lite(full_addr: str="172.16.0.2", full_port: int=6379, lite_addr: str="172.16.0.2", lite_port: int=6479, duration: int=60):
    with open('redis_monitoring.csv', 'w', newline='') as csvfile:
        start_time = time.time()
        writer = csv.writer(csvfile)
        writer.writerow(['Timestamp', 'Full CPU Usage', 'Full Memory Usage', 'Full Throughput', 'Lite CPU Usage', 'Lite Memory Usage', 'Lite Throughput'])

        last_time = time.time()
        last_full_cmd_done = utils.cmd_done_cnt(full_addr, full_port)
        # last_lite_cmd_done = utils.cmd_done_cnt(lite_addr, lite_port)
        
        while time.time() - start_time <= duration:
            full = utils.get_process("redis-server", full_port)
            lite = utils.get_process("redis-lite", lite_port)
            
            full_cmd_done = utils.cmd_done_cnt(full_addr, full_port)
            # lite_cmd_done = utils.cmd_done_cnt(lite_addr, lite_port)
            full_throughput = (full_cmd_done - last_full_cmd_done) / (time.time() - last_time) if full_cmd_done - last_full_cmd_done > 0 else 0
            # lite_throughput = (lite_cmd_done - last_lite_cmd_done) / (time.time() - last_time) if lite_cmd_done - last_lite_cmd_done > 0 else 0
            lite_throughput = full_throughput
            
            last_time = time.time()
            last_full_cmd_done = full_cmd_done
            # last_lite_cmd_done = lite_cmd_done
