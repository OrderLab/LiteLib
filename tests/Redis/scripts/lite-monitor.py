    ...
    with open(csv_file, mode='w') as file:
        ...
        while time.time() - start_time < duration:
            ...
            if lite_process:
                ...
                lite_cpu = lite_mem = None
            
            if redis_process:
                redis_cpu = redis_process.cpu_percent(interval=0.5)
                redis_mem = redis_process.memory_info().rss
            else:
                redis_cpu = redis_mem = None
            
            writer.writerow([timestamp, lite_cpu, lite_mem, redis_cpu, redis_mem])
            print(f"{timestamp}: lite_cpu={lite_cpu}, lite_mem={lite_mem}, redis_cpu={redis_cpu}, redis_mem={redis_mem}")
            time.sleep(0.5)

if __name__ == '__main__':
    get_cpu_mem_usage("data/lite_cpu_mem.csv", 60)