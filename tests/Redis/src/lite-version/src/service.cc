...
std::optional<std::vector<std::shared_ptr<Packet>>> Redis::Filter(
...
void Redis::NormalUpdateImpl(const std::shared_ptr<Packet> &p, Cache &cache, const bool in_transaction)
{
...
Packet Redis::EmergencyServe(std::shared_ptr<Packet> p, ConnectionInfo &conn, Cache &cache, Logger &logger)
{
...
RESPType *Redis::EmergencyServeImpl(std::shared_ptr<Packet> p, ConnectionInfo &conn, Cache &cache, Logger &logger,
