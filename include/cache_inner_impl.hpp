...
bool CacheInner<
  ...
  bip::offset_ptr<LogEntryInstance> old_dirty_node = nullptr;

  bool ret = cache_.visit(key, [&](auto &element) {
...
bool CacheInner<Application, Request, Response, ConnectionInfo, CacheKey,
  ...
  auto inserted = cache_.emplace_and_visit(
      std::piecewise_construct, std::forward_as_tuple(key),
      std::forward_as_tuple(key, value, dirty_node, this, new_size),
      [&](auto &element) {
        new_state = element.second.state;
        lru_node = element.second.lru_node.get();
      },
