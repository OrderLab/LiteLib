...
def main():
    ...
    for axis, (application, modes) in zip(axes, data.items()):
        baseline = float(modes["full"])
        names = list(modes)
        values = [float(modes[name]) / baseline for name in names]
        bars = axis.bar(range(len(names)), values, color="#4c78a8")
        ...
        axis.set_ylabel("Latency / baseline")
